...
/*   Updated: 2025/07/18 16:57:35 by lengarci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/cub3d.h"

static int	read_from_client(int client_fd, char *buffer, t_game *game)
{
	fd_set			readfds;
...
static int	handle_client_read(t_game *game, char *buffer)
{
	...
	bytes_read = read_from_client(game->server.client_fd, buffer, game);
	if (bytes_read == 0)
		return (0);
	if (bytes_read < 0)
	{
