...
/*   Updated: 2025/07/18 16:58:12 by lengarci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	receive_map_and_start_threads(t_game *game)
{
	...
	FD_ZERO(&read_fds);
	FD_SET(game->sock, &read_fds);
	timeout.tv_sec = 0;
	timeout.tv_usec = 1000;
	select_result = select(game->sock + 1, &read_fds, NULL, NULL, &timeout);
	if (select_result < 0)
		destroy_game_failure(game, "Failed to select socket");
	if (select_result == 0)
		destroy_game_failure(game, "Timeout while waiting for map");
