...
Config::Config(const ConfigParse& configParse, size_t index) {
	...
	if (serverConfig.find("client_max_body_size") != serverConfig.end()) {
		...
		{
			// std::cerr << "[Config] Setting client_max_body_size to " << size << " MB" << std::endl;
			size *= 1024 * 1024;
			// std::cerr << "[Config] client_max_body_size in bytes: " << size << std::endl;
		}
		else if (unit == 'G')
			size *= 1024 * 1024 * 1024;
...
Location::Location(const std::map<std::string, std::string>& locConf) {
	...
	it = locConf.find("rootl");
	if (it == locConf.end())
		_root = ""; // default root
	else
		_root = it->second;

	it = locConf.find("indexl");
	if (it == locConf.end())
		_index = ""; // default index
	else
		_index = it->second;
	...
	it = locConf.find("client_max_body_sizel");
	if (it == locConf.end())
		_clientMaxBodySize = 0; // default 1MB
	else
	{
	...
	it = locConf.find("return");
	if (it == locConf.end())
		_return = ""; // default no return
	else
		_return = it->second;
}
...
//generate an user if who have 6 alphanumeric characters
static std::string generateUserId()
{
	...
	for (int i = 0; i < len; ++i)
	{
		userid += alphanum[rand() % (sizeof(alphanum) - 1)];
	}
	return userid;
}

void Config::createSession(Response &res)
{
	std::string sessionid = generateSessionId();
	std::string userid = generateUserId();
	std::cout << "[Session] New session created: " << sessionid << " for user: " << userid << std::endl;
	_session[sessionid] = "user=" + userid + ";";
	res.addSetCookie("sessionid=" + sessionid + "; Path=/; HttpOnly");
