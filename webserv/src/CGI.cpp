...
{
	// std::cerr << "relPath: '" << relPath << "', cgiExt: '" << cgiExt << "'" << std::endl;
	if (cgiExt.empty()) {
		return std::string();
	}
					  ...
					  std::string& outScriptName, std::string& outRequestUri)
{
	...
	if (req.getHeader("Cookie") != "")
		kv.push_back("HTTP_COOKIE=" + req.getHeader("Cookie"));
	else
		kv.push_back("HTTP_COOKIE=");
	if (d.cgiExt == ".php")
		kv.push_back("REDIRECT_STATUS=200");
...
static void check_cookies(const std::string &header, Response &res) {
	size_t pos = 0;
	while (pos < header.size()) {
		...
		if (!line.empty()) {
			if (line.rfind("Set-Cookie:", 0) == 0) {
				size_t sp = line.find(' ');
				if (sp != std::string::npos) {
					std::string cookie = line.substr(sp + 1);
					res.addSetCookie(cookie);
					std::cout << "Added Set-Cookie: " << cookie << std::endl;
				}
			}
		}
...
Response CGI::run(const Router::Decision &decision, const Request &req) {
	...
	if (pid == 0)
	{
		...
		Router::Decision tmp = decision;
		tmp.reason = "Failed to execute CGI script";
		res = StaticExec::makeError(tmp, *decision.server);
		close(pipe_in[0]);
		close(pipe_out[1]);
		_exit(1);
	}
	...
	if (raw.empty()) {
		Router::Decision tmp = decision;
		tmp.reason = "No output from CGI script";
		res = StaticExec::makeError(tmp, *decision.server);
		return res;
	}
	if (!parseCgiOutput(res, raw)) {
		Router::Decision tmp = decision;
		tmp.reason = "Malformed CGI output";
		res = StaticExec::makeError(tmp, *decision.server);
		return res;
	}
	if (res.getStatusCode() > 299) {
		// error status from CGI
		Router::Decision tmp = decision;
		tmp.status = res.getStatusCode();
		tmp.reason = res.getStatusMessage().empty() ? "CGI Error" : res.getStatusMessage();
		res = StaticExec::makeError(tmp, *decision.server);
		// std::cout << "CGI error status: " << tmp.status << " " << tmp.reason << std::endl;
		std::cerr << "[DEBUG RAW] " << raw << std::endl;
		return res;
	}
	// res.setStatus(200, "OK");
