...
void ConfigParse::tokenize() {
	...
	while (pos < _fileContent.size()) {
		...
		if (isalnum(_fileContent[pos]) || _fileContent[pos] == '/' || _fileContent[pos] == '.' || _fileContent[pos] == '_' || _fileContent[pos] == '?') {
			size_t start = pos;
			...
			|| _fileContent[pos] == '?' || _fileContent[pos] == '&' || _fileContent[pos] == '=' || _fileContent[pos] == '~'))
				pos++;
...
void ConfigParse::parseLocationBlock(size_t &i, size_t &serverCount) {
	...
	while (i < _tokens.size() && _tokens[i].type != T_RBRACE) {
		...
		if (_tokens[i].type == T_STRING && _tokens[i - 2].value == "return") {
			value = _tokens[i - 1].value + " " + _tokens[i].value;
