...
private:
	...
	std::map<std::string, std::string> _session ; // session storage (for cookies)
public:
	...
	void createSession(Response &res);
	const std::string &getSession(const std::string &id) const { return _session.at(id); }
};
