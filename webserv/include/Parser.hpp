...
/*   Updated: 2025/09/23 09:29:30 by macauchy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
class Parser
{
	public:
		...
		bool	feed( const std::string &data, const Config& cfg);	// Feed raw data to the parser
	...
	private:
		...
		bool		_lastChunkSeen;			// true when we have seen the 0 chunk and must parse trailers
		...
		void	_parseRequestLine( const std::string &line, const Config &cfg);
		void	_parseHeaderBlock( const std::string &rawHeaders );
		bool	_parseHeaderLine( const std::string &line, std::string &key, std::string &value );
		bool	_parseBody( void );
		bool	_decodeChunked( void );
		...
		bool		_parseHex( const std::string &s, unsigned long long &out ) const;
